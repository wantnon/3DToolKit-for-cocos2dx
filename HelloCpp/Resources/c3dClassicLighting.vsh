...
void main(void) {
    ...
    //suppose scaling is uniformed, so we can use modelMat as normalMat
    //suppose no scaling, so we need not normalization
    vec3 norm_world = vec3(modelMat*a_normal);
